...
import me.capitainecat0.multicommands.commands.implementers.BanIP;
            ...
            if (sender instanceof Player) {
                if (!perms.hasPermission((OfflinePlayer) sender, BAN_PERM.getPermission())
                    ...
                    if (args.length <= 1) {
                        ...
                        if (!target.isBanned()) {
                            new BanIP(target, sender, args).execute();
                ...
                if(args.length <= 1){
                    ...
                    OfflinePlayer target = Bukkit.getOfflinePlayer(uuid);
                    if (!target.isBanned()) {
                        new BanIP(target, sender, new String[]{getBanReason(args)}).execute();
