...
import me.capitainecat0.multicommands.commands.implementers.Freeze;
...
import static me.capitainecat0.multicommands.utils.permissions.Perms.FREEZE_PERM;
...
public class FreezeCMD implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
        ...
        try {
            if (!perms.hasPermission((Player) sender, FREEZE_PERM.getPermission())
                ...
                if (args.length == 0) {
                    ...
                    if (target != null) {
                        new Freeze((Player) sender, target).execute();
                    } else {
                        playSoundIfEnabled(sender, Sound.valueOf(MultiCommands.getInstance().getConfig().getString("no-perm-sound")), 1f, 1f);
