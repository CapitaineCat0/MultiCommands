...
public class BanIP implements CommandExecutor {
    ...
    /**
     *
     * The BanIP command ban players that operators targeting by IP
        ...
        try {
            if(args.length <= 1){
                ...
                if(!target.isBanned()){
                    ...
                    getInstance().getServer().getBanList(BanList.Type.IP).addBan(Objects.requireNonNull(target.getName()), BAN_PREFIX.getMessage().replace("{0}", banReason), null, sender.getName());
    ...
    /**
     * The notifyOperators function is used to notify all online operators that a player has been banned.
     *
     *
     * @param target Get the name of the player that was banned
     * @param sender Get the name of the command sender
     * @param banReason Replace {2} in the ban_admin message
     */
