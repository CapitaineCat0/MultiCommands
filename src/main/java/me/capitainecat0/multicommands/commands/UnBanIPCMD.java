...
public class UnBanIPCMD implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        try{
            if(sender instanceof Player){
                if(!perms.hasPermission((Player) sender, UNBANIP_PERM.getPermission())
                    ...
                    if(target.isBanned()){
                        data.remove(target);
                        getInstance().getServer().getBanList(BanList.Type.IP).pardon(Objects.requireNonNull(target.getAddress()).toString());
