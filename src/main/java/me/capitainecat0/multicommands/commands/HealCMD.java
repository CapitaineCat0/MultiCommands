...
import me.capitainecat0.multicommands.commands.implementers.Heal;
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        PermissionManager perms = MultiCommands.getPermissionManager();
            if(sender instanceof Player){
                try{
                    if(args.length == 0){
                        if(perms.hasPermission((Player) sender, HEAL_PERM_SELF.getPermission())
                                || perms.hasPermission((Player) sender, HEAL_PERM_ALL.getPermission())
                                || perms.hasPermission((Player) sender, ALL_PERMS.getPermission())){
                            if(((Player) sender).getHealth() != 20){
                                new Heal((Player) sender, null).execute();
                            }else{
                ...
                if(args.length == 1){
                    try{
                        if(perms.hasPermission((Player) sender, HEAL_PERM_OTHER.getPermission())
                                || perms.hasPermission((Player) sender, HEAL_PERM_ALL.getPermission())
                                || perms.hasPermission((Player) sender, ALL_PERMS.getPermission())){
                            Player target = Bukkit.getPlayerExact(args[0]);
                            if(target != null){
                                if(target.getHealth() != 20){
                                    new Heal((Player) sender, target).execute();
                                }else{
                                    playSoundIfEnabled(sender, Sound.valueOf(MultiCommands.getInstance().getConfig().getString("no-perm-sound")), 1f, 1f);
