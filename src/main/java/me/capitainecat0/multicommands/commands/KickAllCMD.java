...
import me.capitainecat0.multicommands.commands.implementers.Kick;
...
public class KickAllCMD implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        Player onlinePlayer = (Player) Bukkit.getOnlinePlayers();
        if(sender instanceof Player){
            try{
                if(perms.hasPermission((Player) sender, KICKALL_PERM.getPermission())
                    ...
                    if(args.length <= 1){
                        if (!onlinePlayer.hasPermission(KICKALL_EXEMPT_PERM.getPermission())) {
                            new Kick(onlinePlayer, args).execute();
                        }
                        playSoundIfEnabled(sender, Sound.valueOf(MultiCommands.getInstance().getConfig().getString("no-perm-sound")), 1f, 1f);
