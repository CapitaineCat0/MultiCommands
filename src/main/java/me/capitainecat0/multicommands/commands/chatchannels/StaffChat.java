    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String @NotNull [] args) {
        ...
        if (args.length >= 1) {
            try {
                ...
                boolean hasPermission = MultiCommands.getPermissions().has((Player) sender, Perms.STAFFCHAT_PERM.getPermission()) || MultiCommands.getPermissions().has((Player) sender, ALL_CHAT_PERM.getPermission()) || MultiCommands.getPermissions().has((Player) sender, ALL_PERMS.getPermission());
