...
public class DevChatCMD implements CommandExecutor, Listener {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String @NotNull [] args) {
        ...
        if (args.length >= 1) {
            try {
                if(!perms.hasPermission((Player) sender, DEVCHAT_PERM.getPermission())
                ...
                new ChatChannels(DEVCHAT_PERM.getPermission()).broadcast(format);
